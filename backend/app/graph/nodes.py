...
def parse_alert_node(state: IncidentState) -> Dict[str, Any]:
    ...
    alert_type = (
        raw.get("alert_type")
        or raw.get("rule", {}).get("description", "unknown")
    )
    ...
    ioc_domain = (
        raw.get("ioc_domain")
        or raw.get("dns", {}).get("question", {}).get("name")
    )

    ioc_hash = (
        raw.get("ioc_hash")
        or raw.get("file", {}).get("hash", {}).get("sha256")
    )
...
async def asset_context_node(
    ...
    if asset_id:
        ctx = await StructuredRetrieval.asset_context(
            db,
            asset_id,
        ) or {}
        ...
        try:
            paths = await neo4j_service.find_correlation_paths(
                asset_id,
                max_depth=2,
            )
...
async def structured_history_node(
    state: IncidentState,
    db: AsyncSession,
) -> Dict[str, Any]:
    """
    Text-to-SQL agent: have we seen this IOC / asset /
    alert_type combination before?
    """
...
async def vector_retrieval_node(
    state: IncidentState,
    db: AsyncSession,
) -> Dict[str, Any]:
    """
    Hybrid BM25 + vector search with RRF across
    postmortem + CVE embeddings.
    """
    ...
    query_text = (
        f"{alert_type} {ioc_ip} {ioc_domain} intrusion alert"
    ).strip()
    ...
    cves = await HybridRetrieval.search_cves(
        db,
        query_text,
        embedding,
        top_k=20,
    ...
    postmortems = await HybridRetrieval.search_postmortems(
        db,
        query_text,
        embedding,
        top_k=20,
...
async def mitre_mapping_node(
    ...
    query_text = (
        f"MITRE ATT&CK technique for {alert_type}"
    )
    ...
    techniques = await HybridRetrieval.search_mitre(
        db,
        embedding,
        top_k=5,
    )
...
def correlation_reasoner_node(
    state: IncidentState,
) -> Dict[str, Any]:
    """
    LLM reasons over all context, drafts verdict +
    cited reasoning trace.
    """
    ...
    incidents_text = json.dumps(
        incidents[:5],
        indent=2,
        default=str,
    )

    cves_text = json.dumps(
        cves[:5],
        indent=2,
        default=str,
    )

    techniques_text = json.dumps(
        techniques[:5],
        indent=2,
        default=str,
    )
...
Your job is to classify an incoming SIEM alert as one of:
...
"""
    ...
    return {
        ...
        "confidence": parsed_response.get(
            "confidence",
            0.0,
        ),
        "reasoning": parsed_response.get(
            "reasoning",
            "",
        ),
        "_cited_incident_ids": parsed_response.get(
            "cited_incident_ids",
            [],
        ),
        "_cited_cve_ids": parsed_response.get(
            "cited_cve_ids",
            [],
        ),
        "_cited_technique_ids": parsed_response.get(
            "cited_technique_ids",
            [],
        ),
    }
...
def grounding_verifier_node(
    state: IncidentState,
) -> Dict[str, Any]:
    """
    Check every claim cites a real ID returned by retrieval.
    If not, loop back.
    """
    ...
    bad_incidents = (
        cited_incidents - valid_incident_ids
    )

    bad_cves = cited_cves - valid_cve_ids

    bad_techniques = (
        cited_techniques - valid_technique_ids
    )
    ...
    if not grounded and retry < 2:
        ...
        return {
            ...
            "reasoning": (
                state.get("reasoning", "")
                + "\n[GROUNDING FAIL: retries exhausted; "
...
def severity_classifier_node(
    state: IncidentState,
) -> Dict[str, Any]:
    """
    Combine asset criticality + CVE CVSS +
    historical severity -> final severity.
    """
...
async def audit_logger_node(
    ...
    """
    Persist correlation results and audit information.

    Additionally creates a real Incident record when the
    correlation determines that the alert requires investigation.
    ...
    Duplicate incidents are prevented by checking the alert ID
    in the incident title before creating a new record.
    """
    ...
    # ---------------------------------------------------------
    # 3. Determine whether an incident should be created
    # ---------------------------------------------------------
    ...
    if should_create_incident:
        ...
        incident_title = (
            f"{severity.upper()} Security Incident — "
            f"{alert_type} — Alert {str(alert_id)[:8]}"
        )

        # -----------------------------------------------------
        # 4. Prevent duplicate incident creation
        # -----------------------------------------------------
        ...
        if existing_incident:
            created_incident = existing_incident
            ...
            description = (
                "SentinelIQ automatically created "
                "this incident from a correlated SIEM alert.\n\n"
                f"Alert ID: {alert_id}\n"
                ...
                f"Grounding Passed: "
                f"{state.get('grounding_passed', False)}\n\n"
                "Reasoning:\n"
                f"{reasoning}\n\n"
                "Correlation Report:\n"
                f"{report}"
            ...
            # Get UUID before committing so we can
            # include it in the audit metadata.
            await db.flush()
    ...
    audit_metadata = {
        ...
        "incident_created": (
            created_incident is not None
        ),
    }

    if created_incident:
        audit_metadata["incident_id"] = str(
            created_incident.id
        )
    ...
    # ---------------------------------------------------------
    # 6. Commit correlation + incident + audit atomically
    # ---------------------------------------------------------
